...
HINT ε₁ ✱ [ acc ; (p +ₗ 0) ↦ #x ∗ (p +ₗ 1) ↦ l_acc ∗ is_list acc l_acc ∗ ⌜t = x :: acc⌝]
  ⊫ [id]; is_list t #p ✱ [⌜t = x :: acc⌝].
Proof. unfold is_list, array. iSteps. Qed.
...
Proof.
  ...
  - iModIntro. iIntros "[%xs' [% [% [% [? [Hxs [? [? ->]]]]]]]]".
    destruct xs'; iDecompose "Hxs"; wp_type.  (* Split the current list as in the `match` statement. *)
